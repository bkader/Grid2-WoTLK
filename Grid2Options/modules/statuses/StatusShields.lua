...
Grid2Options:RegisterStatusOptions("shields-overflow", "health", nil, {title = L["display damage absorb shields above max hp"], titleIcon = "Interface\\ICONS\\Spell_Holy_PowerWordShield"})
Grid2Options:RegisterStatusOptions(
	"shields",
	"health",
	function(self, status, options, optionParams)
		self:MakeStatusColorOptions(
			status,
			options,
			{
				color1 = L["Normal"],
		...
		options.maxShieldAmount = {
			...
			step = 1,
			get = function()
				return status.dbx.maxShieldValue or 0
			end,
			set = function(_, v)
		...
		options.thresholdMedium = {
			...
			step = 1,
			get = function()
				return status.dbx.thresholdMedium
			end,
			set = function(_, v)
				if status.dbx.thresholdLow > v then
					v = status.dbx.thresholdLow
				end
		...
		options.thresholdLow = {
			...
			step = 1,
			get = function()
				return status.dbx.thresholdLow
			end,
			set = function(_, v)
				if status.dbx.thresholdMedium < v then
					v = status.dbx.thresholdMedium
				end
